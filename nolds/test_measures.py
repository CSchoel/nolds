        ...
        for i in range(10):
            ...
            for i in reversed(range(max(1, min_len - 5), min_len + 5)):
                ...
                if i < min_len:
                    ...
                    try:
                        assert np.all(np.isfinite(nolds.lyap_r(data, fit="poly", **kwargs))), (
        ...
        for i in range(10):
            ...
            for i in reversed(range(max(1, min_len - 5), min_len + 5)):
                ...
                if i < min_len:
                    ...
                    msg = "{} data points should be enough for kwargs {}, but  {} where too few"
                    try:
                        assert np.all(np.isfinite(nolds.lyap_e(data, **kwargs))), msg.format(
                            min_len, kwargs, i
                        )
                    except ValueError as e:
                        self.fail(
...
class RegressionTests(unittest.TestCase):
    ...
    def test_sampen(self) -> None:
        ...
        se = nolds.sampen(
            data, emb_dim=2, tolerance=None, lag=1, dist=nolds.rowwise_chebyshev, closed=False
        )
    ...
    def test_corr_dim(self) -> None:
        ...
        data = datasets.load_qrandom()[:1000]
        cd = nolds.corr_dim(
            data, emb_dim=5, lag=1, rvals=None, dist=nolds.rowwise_euclidean, fit="poly"
        )
    ...
    def test_corr_dim_RANSAC(self) -> None:
        ...
        cd = nolds.corr_dim(
            data, emb_dim=5, lag=1, rvals=rvals, dist=nolds.rowwise_euclidean, fit="RANSAC"
        )
