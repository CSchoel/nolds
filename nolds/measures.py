...
def poly_fit(
    ...
    if fit == "RANSAC":
        try:
            ...
            warnings.warn(
                "fitting mode 'RANSAC' requires the package sklearn, using 'poly' instead",
                RuntimeWarning,
                stacklevel=2,
            )
            fit = "poly"
...
def lyap_r(  # noqa: C901, PLR0912, PLR0915
    ...
    *,
    ...
    if len(data) < min_len:
        msg = (
            "for emb_dim = {}, lag = {}, min_tsep = {} and trajectory_len = {}"
...
def lyap_e(  # noqa: C901, PLR0915
    ...
    *,
    ...
    if n < min_len:
        msg = (
            "{} data points are not enough! For emb_dim = {}, matrix_dim = {}"
    ...
    if len(orbit) < min_nb:
        ...
        msg = (
            "Not enough data points. Need at least {} additional data points "
    ...
    for i in range(len(orbit)):
        ...
        if np.isinf(r):
            assert len(data) < min_len
            msg = (
                "Not enough data points. Orbit vector {} has less than min_nb = "
...
def logarithmic_r(min_r: float, max_r: float, factor: float) -> list[float]:
    ...
    Args:
        min_r: minimum value (must be < max_r)
        max_r: maximum value (must be > min_r)
        factor: factor used to increase min_r (must be > 1)

    Returns:
        [min_r, min_r * factor, min_r * factor^2, ... min_r * factor^i] where
        all values are < max_r.
    """
    if max_r <= min_r:
        msg = f"max_r must be larger than min_r ({max_r} <= {min_r})."
        raise ValueError(msg)
    if factor <= 1:
        msg = f"Factor must be larger than 1, but got {factor}."
        raise ValueError(msg)
    max_i = int(np.floor(np.log(1.0 * max_r / min_r) / np.log(factor)))
...
def plot_reg(
    ...
    y_label: str = "y",
    data_label: str = "data",
    reg_label: str = "regression line",
    fname: str | Path | None = None,
) -> None:
    """Plots trend lines for line-fitting approaches.

    This function will show a plot through ``plt.show()`` and close it after the
    window has been closed by the user unless `fname` is provided, in which case
    the plot will be saved to disc under the given file name instead.

    Args:
        xvals: list of x-values
        yvals: list of y-values
        poly: polynomial parameters as accepted by ``np.polyval``
        x_label: label of the x-axis
        y_label: label of the y-axis
        data_label: label of the data
        reg_label: label of the regression line
        fname: file name (if not None, the plot will be saved to disc instead of
...
def plot_reg_tiled(
    ...
    """Plots trend lines for multiple line-fitting approaches in a tiled layout.
    ...
    Args:
        xvals: values on the x-axis in shape (#plots, #datapoints)
        yvals: values on the y-axis in shape (#plots, #datapoints)
        polys: polynomial parameters as accepted by ``np.polyval`` in shape
            (#plots, #params)
        x_label: x axis label
        y_label: y axis label
        data_labels: labels of the povided datasets
        reg_labels: labels of the regression lines
        fname: file name (if not None, the plot will be saved to disc instead of
            showing it though ``plt.show()``)
        columns: number of columns for the tiled view, defaults to minimum
            number required to obtain a square grid.
    """
    # local import to avoid dependency for non-debug use
    import matplotlib.pyplot as plt

    max_span = max([float(np.max(y)) - float(np.min(y)) for y in yvals])
...
def plot_reg_multiple(
    ...
    """Plots trend lines for multiple line-fitting approaches in a the same plot.
...
def hurst_rs(
    ...
    *,
    ...
    Args:
        data: time series
        nvals: sizes of subseries to use
        ...
        plot_file: if debug_plot is True and plot_file is not None, the plot will be saved
            under the given file name instead of directly showing it through
            ``plt.show()``
        corrected: if True, the Anis-Lloyd-Peters correction factor will be applied to the
    ...
    Returns:
        ...
        * the Hurst exponent K
        * a tuple of three arrays:
            - nvals: the values used for n
...
def mfhurst_b(
    ...
    *,
    ...
    r"""Calculates the Generalized Hurst Exponent H_q according to A.-L. Barabási and T. Vicsek.
    ...
    Args:
        data: time series of data points (should be evenly sampled)
        qvals: values of q for which H_q should be calculated (default: [1])
        dists: distances for which the height-height correlation should be calculated
    ...
    Returns:
        ...
        the return value is instead a tuple containing

        * the H_q values for each q in ``qvals``
        * a tuple of three arrays
            ...
            - yvals: the logarithm of the height-height correlations for each
                distance (first dimension) and each q (second dimension) with shape
                (len(dists), len(qvals))
            - poly: the coefficients of the line fit (``[slope, intercept]``)
    ...
    if len(data) < min_reliable_n:
        warnings.warn(
            f"H(q) is not reliable for small time series ({len(data)} < 60)",
            stacklevel=2,
        )
...
def mfhurst_dm(
    ...
    *,
    ...
    This implementation is a port of the MATLAB code of Tomaso Aste.
    ...
    Args:
        data: input data (should be evenly sampled)
        qvals: values of q for which H_q should be calculated (default: [1])
        max_dists: different values to test for tau_max, the maximum value for the distance
            d. The resulting H_q will be a mean of all H_q calculated with tau_max
            = max_dists[0], max_dists[1], ... .
        detrend: if True, a linear trend will be removed from the data before H_q will
            be calculated
        fit: the fitting method to use for the line fit, either 'poly' for normal
            least squares polynomial fitting or 'RANSAC' for RANSAC-fitting which
            is more robust to outliers
...
def corr_dim(
    ...
    *,
    ...
    Returns:
        ...
        - debug_data: tuple containing
            - rvals: the values used for log(r)
